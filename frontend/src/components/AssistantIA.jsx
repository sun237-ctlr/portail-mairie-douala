...
export default function AssistantIA() {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: 'Bonjour ! Je suis l\'assistant de la Mairie de Douala 🏛️. Comment puis-je vous aider aujourd\'hui ?'
    }
  ]);
  ...
  const envoyerMessage = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/ia/chat', {
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-green-700 text-white rounded-full shadow-lg hover:bg-green-800 transition flex items-center justify-center text-2xl z-50">
        {ouvert ? '✕' : '💬'}
      </button>

      {/* Fenêtre chat */}
      {ouvert && (
        <div className="fixed bottom-24 right-6 w-80 md:w-96 bg-white rounded-2xl shadow-2xl border border-gray-200 z-50 flex flex-col"
          style={{ height: '500px' }}>

          {/* Header */}
          <div className="bg-green-700 text-white px-4 py-3 rounded-t-2xl flex items-center gap-3">
            <div className="w-8 h-8 bg-green-600 rounded-full flex items-center justify-center text-sm">🤖</div>
            <div>
              <p className="font-semibold text-sm">Assistant Mairie</p>
          ...
          <div className="p-3 border-t border-gray-100 flex gap-2">
            ...
            <button onClick={envoyerMessage} disabled={chargement || !input.trim()}
              className="bg-green-700 text-white w-9 h-9 rounded-xl flex items-center justify-center hover:bg-green-800 disabled:opacity-40 transition">
              ➤
            </button>
          </div>
        </div>
