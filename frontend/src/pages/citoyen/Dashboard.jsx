...
const statutConfig = {
  ...
  RENDEZ_VOUS_PROGRAMME: { label: 'RDV programmé', couleur: 'bg-purple-100 text-purple-700', Icon: Clock },
                ...
                return (
                  <div key={demande.id} className="p-5 hover:bg-gray-50 transition">
                    ...
                    {demande.dateRendezVous && (
                      <div className="mt-3 bg-purple-50 border border-purple-100 rounded-xl px-4 py-2.5 text-sm text-purple-700">
                        🏛️ Rendez-vous le : <strong>{new Date(demande.dateRendezVous).toLocaleDateString('fr-FR')}</strong>
                      </div>
                    )}
                    {demande.dateDisponibilite && (
                      <div className="mt-3 bg-green-50 border border-green-100 rounded-xl px-4 py-2.5 text-sm text-green-700">
                        🎉 Acte disponible le : <strong>{new Date(demande.dateDisponibilite).toLocaleDateString('fr-FR')}</strong>
                      </div>
                    )}
                  </div>
                );
